    ...
    while iter < max_iter
        ...
        if is_solved_and_feasible(master)
            ...
            if !isnothing(cols) && !isempty(cols)
                ...
                # Filter only columns with negative reduced cost (col.cost is the reduced cost from labeling)
                improving_cols = filter(x -> x.cost < -1e-6, cols)
                ...
                println("  Labeling found ", length(cols), " columns, ", length(improving_cols), 
                        " with negative reduced cost, adding best ", length(cols_to_add))
...
function solve_integer_master(master::Model, data::Data)
    ...
    set_attribute(int_master, "TimeLimit", 600)  # 10 minutes
    set_attribute(int_master, "LogToConsole", 0)  # Enable logging to see progress
    set_attribute(int_master, "MIPGap", 0.001)  # 0.1% optimality gap
    set_attribute(int_master, "MIPFocus", 0)  # Tries to balance finding good solutions with proving optimality
...
function branch_and_price(data::Data)
    ...
    run_cnt = 1
    max_nodes = 500 # Add max nodes limit to prevent infinite branching
...
################### Run the B-P ################################
################################################################
input_list = ["E-n51-k5.csv", 5, 160.0]
